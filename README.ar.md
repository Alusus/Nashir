...
* `خيارات_الخادم: مـصفوفة[مـؤشر_محارف]` (`serverOptions: Array[CharsPtr]`) 
  
   يستخدم هذا المعطى لتحديد
  معطيات خادم Http في حال طلبنا من الناشر إنشاء الدالتين الرئيسيتين.

* `مسار_الموارد_الرئيسية_لمنصة_ويب: نـص` (`webPlatformMainAssetsPath: String`) 

   مسار الموارد الرئيسية
  الذي تحتاج إليه مكتبة مـنصة_ويب. يُهمل في حال لم يُطلب من الناشر إنشاء الدالتين الرئيسيتين.

* `مسار_المنافذ_المرئية_لمنصة_ويب: نـص` (`webPlatformUiEndpointsPath: String`) 

   مسار ملفات المنافذ
  المرئية الذي تحتاج إليه مكتبة مـنصة_ويب. يُهمل في حال لم يُطلب من الناشر إنشاء الدالتين الرئيسيتين.

* `الموارد: تـطبيق[نـص، نـص]` (`assets: Map[String, String]`) 
  
   قائمة بمسارات الموارد التي يستخدمها البرنامج.
...
* `الاعتماديات: مـصفوفة[نـص]` (`dependencies: Array[String]`) 

   قائمة المكتبات التي يحتاج إليها المشروع
  أثناء البناء.

* `مشغل_النشر: سـندنا[مـشغل_نشر]` (`publishDriver: SrdRef[PublishDriver]`) 

  المشغل المُستخدم لنشر المشروع.

* `وحدات_الخادم: سـند[كـائن_بهوية]` (`serverModules: ref[TiObject]`) 
  
   سند إلى الوحدة أو الوحدات التي
  تحتاج إليها مـنصة_ويب أثناء تشغيل وبناء المشروع.

* `بداية_آنية: سـند[كـائن_بهوية]` (`startJit: ref[TiObject]`) 

  سند إلى دالة تشغيل الخادم المخصصة للتشغيل
...
* `بداية_مبنية: سـند[كـائن_بهوية]` (`startExe: ref[TiObject]`) 

   سند إلى دالة مدخل البرنامج المستخدمة في
...
* `مهيئ: سـند[كـائن_بهوية]` (`initializer: ref[TiObject]`)
  
  . معطى اختياري، وهو سند إلى دالة تهيئة ستُستدعى
...
كما يحتوي المشغل على معطى `الباقة_المطلوبة: نـص` (`desiredPlan: String`) يمكن ضبطه لتحديد باقة
الاشتراك المطلوبة عند نشر المشروع على الأسس نت.
...
حقوق النشر © 2026 الأسس للبرمجيات (Alusus Software Ltd)
